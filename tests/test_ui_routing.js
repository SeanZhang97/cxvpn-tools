...
assert.match(html, /id="btn-routing-test-region"/);
assert.match(html, /id="btn-routing-test-all"/);
assert.match(html, /id="routing-test-region-count"/);
assert.match(html, /id="routing-test-all-count"/);
assert.match(html, />开始测速</);
assert.match(source, /let nodeTestScope = 'all'/);
assert.match(source, /function batchTestTarget/);
assert.match(source, /start_preview_routing_test\(\{ \.\.\.provider \}, \{/);
assert.match(source, /start_routing_group_test\(groupId, targets\)/);
assert.match(source, /单节点 · \$\{nodeTools\.nodeText\(node\)\}/);
assert.match(css, /\.routing-test-scope button\.active/);
assert.ok(css.includes('.routing-test-scope button:not(.active):hover:not(:disabled)'));
assert.ok(!css.includes('.routing-test-scope button:hover:not(:disabled)'));
assert.match(css, /\.routing-node-action-buttons/);
