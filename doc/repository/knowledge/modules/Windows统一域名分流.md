...
- 订阅行的“查看节点”经 `setRoutingTab` 调用 `ProxyWorkspace.openNodes`。该函数在页面切换与异步加载之前保存目标分组，加载后用此目标选中并同步自定义下拉框；不能在加载后读取首页共享的 `selectedGroupId`，因为页面事件与加载回调中的 `sync` 会按默认出口重置该变量。
...
- 初次预览只证明订阅可下载且节点格式可解析，节点页先把结果标为“未测速”。用户可在节点页
  执行临时测速：控制面以相同物理接口和 DNS 临时启动不含 TUN、监听代理端口和
...
- 代理节点页按代理组展示节点名称、类型、可用状态和最近延迟，支持搜索、只看可用、单节点测速，
  以及“当前地区 / 全部节点”两档批量测速范围。选择具体地区时默认切换到当前地区范围并显示
  地区节点数；切回“全部”地区时自动使用全部节点范围。前端通过 `start_*_routing_test` 创建任务，
  再轮询 `get_routing_test_job`；任务提供
...
- 运行态整组测速取消后停止提交新节点，只收集已经完成的结果，并与测速前完整节点清单合并，
  未完成节点不会从快照消失；配置变化导致的过期任务不能落盘。单节点和地区测速提交精确节点名，
  后端校验这些节点仍属于当前代理组，只更新本次目标节点的延迟、状态和时间；未选中节点的历史结果
  继续保留。临时预览测速同样保留未选中节点的持久快照。前端进度轮询采用指数退避；持续失败时显式请求取消，取消状态未知则继续低频
