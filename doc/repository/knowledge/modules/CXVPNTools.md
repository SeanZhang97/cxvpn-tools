...
最后验证：2026-09-07，main。
`ui/index.html` 的 `#proxy-orbit` 使用透明背景的内联 SVG 表现环形光轨、
轮廓五角星和四条由底部光源汇聚上行的曲线。曲线在 `defs` 中定义一次，通过
`use` 复用为静态底线和流光，`pathLength=100` 保证不同长度曲线采用一致的动画进度。
四条曲线保留超星标志的扇形展开，五角星采用原有倾斜轮廓；末端靠近星形下缘，
保留窄留白以避免描边粘连，不设置额外连接杆或共用长颈。
底部公共起点 `(80,143)` 位于以 `(80,80)` 为圆心、半径 `63` 的内圈上；两侧充能弧
沿内圈流入该点，随后把流光传给四路曲线。
`ui/style.css` 使用 5.6 秒周期同步内圈供能、底部蓄光、四路错峰上行与星星提亮，光轨以
18 秒周期缓慢旋转；光晕仅覆盖光源、流光和星形周围，主体不使用实心填色。
`ui/proxy.js` 仍只按后端确认的 running 状态切换 `is-active`；未开启和异常状态
保持静态低亮度，减少动态效果偏好下禁用所有组件动画与过渡。旧 PNG 图层不参与渲染。
图标是装饰元素，状态通过旁边的 `aria-live` 文本表达；不承载独立点击行为。
...
`is-active`，由 CSS 表现圆环双向运动、底部充能、路径向上流光、星星点亮和环形脉冲；未运行状态
也保留慢速淡版流光，异常/未知状态降低饱和度；动画不再随系统“减弱动画”设置降级。
...
- 运行日志由 `Api.log` 统一转为单行事件，并通过有界后台写入器追加到 `run.log`；
  队列满时丢弃最旧待写项，退出时由 `Api.shutdown()` 等待有限时间完成落盘，文件按
  2MB 轮转为 `.1/.2`。网络请求日志仍按既有诊断策略保存。
- 网络出口自动刷新仅在总览页每 5 分钟触发；后端在上次部分失败后按 5 分钟起、
  最大 30 分钟指数退避。海外 Cloudflare 探测入口并发竞速，IP 位置/运营商结果缓存
  15 分钟，保留旧值并标记 `stale`。
- VLM 请求默认超时 35 秒；连续 3 次网络/HTTP 失败后熔断 120 秒，成功请求清除
  失败计数。熔断只影响视觉模型调用，验证码仍按既有人工兜底流程继续。
