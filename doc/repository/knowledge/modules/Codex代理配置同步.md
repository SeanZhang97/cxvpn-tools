...
- 关闭系统接管成功后调用 `codex_proxy.restore()`；其语义是关闭 Codex 代理：删除仍等于本工具最后写入值的托管字段，用户手动改过的字段保留并记录警告。正常退出不会因为窗口关闭而删除 Codex 字段，因为现有程序允许系统代理继续保持；系统代理残留清扫仍由 `proxy_guard` 独立负责。
...
`tests/run_routing_offline.py` 聚合执行 `test_codex_proxy`（缺失配置、局部更新、section 补齐、端口变更、幂等、恢复、手动修改保护、损坏 TOML、权限/写入中断、中文/UTF-8、read_config/status）与 `test_api_codex_proxy`（动态取端口、重启提示标记、查看日志脱敏、状态摘要合并）。
